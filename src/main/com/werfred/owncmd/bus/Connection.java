...
public class Connection {
	...
	protected static String readUntilDelimiter(final BufferedReader inputStream) throws IOException,SocketTimeoutException{
		...
		// Cycle that reads one char each cycle and stop when the sequence ends with ## that is the OpenWebNet delimiter of each message
		do{
			ci = inputStream.read();		    		
			if (ci == -1){
	...
	protected static String[] readUntilAckNack(final BufferedReader inputStream) throws IOException{
		...
		// A sequence of multiple messages end always with an ACK or NACK so stop this cycle when the message is one of them
		do{
